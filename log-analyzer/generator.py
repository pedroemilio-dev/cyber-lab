...
ACCOUNT_TAKEOVER_IPS = ["77.81.142.9",   "104.244.76.13",  "5.188.10.180"]
RESOURCE_ENUM_IPS   = ["176.10.99.200",  "62.171.137.5",   "146.70.41.18"]
REGISTER_FLOOD_IPS  = ["91.219.236.4",   "194.61.24.9",    "5.181.235.71"]
...
AVAILABLE_ATTACKS = [
    "brute_force", "sqli", "traversal", "scan", "xss", "cmd_injection",
    "http_tampering", "account_takeover", "resource_enum", "register_flood"
]
...
def _add_account_takeover(entries: list, start: datetime):
    base = start + timedelta(hours=22)
    for ip in ACCOUNT_TAKEOVER_IPS:
        attempts = random.randint(6, 15)
        for i in range(attempts):
            dt = base + timedelta(seconds=i * random.randint(1, 5))
            entries.append((dt, make_entry(
                ip, "PATCH", ACCOUNT_TAKEOVER_PATHS[0], 400,
                random.randint(80, 200), AGENTS[1], dt
            )))
        base += timedelta(hours=1)


def _add_resource_enum(entries: list, start: datetime):
    base = start + timedelta(hours=23)
    for ip in RESOURCE_ENUM_IPS:
        template = random.choice(RESOURCE_ENUM_PATHS)
        attempts = random.randint(10, 25)
        for i in range(1, attempts + 1):
            dt = base + timedelta(seconds=i * random.randint(1, 3))
            entries.append((dt, make_entry(
                ip, "GET", template.format(i), 404,
                random.randint(0, 150), AGENTS[2], dt
            )))
        base += timedelta(hours=1)


def _add_register_flood(entries: list, start: datetime):
    base = start + timedelta(hours=24)
    for ip in REGISTER_FLOOD_IPS:
        attempts = random.randint(8, 20)
        for i in range(attempts):
            dt = base + timedelta(seconds=i * random.randint(1, 4))
            entries.append((dt, make_entry(
                ip, "POST", REGISTER_FLOOD_PATHS[0], random.choice([409, 400]),
                random.randint(80, 200), AGENTS[3], dt
            )))
        base += timedelta(hours=1)
...
def generate(attacks: list[str] = None):
    ...
    attack_map = {
        ...
        "account_takeover": _add_account_takeover,
        "resource_enum":    _add_resource_enum,
        "register_flood":   _add_register_flood,
    }
