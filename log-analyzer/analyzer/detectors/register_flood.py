...
def detect(entries: list[LogEntry]) -> list[Alert]:
    ...
    for entry in entries:
        is_apache_match = (
            entry.method == "POST" and "/register" in entry.path and entry.status in (400, 409)
        )
        is_spring_match = entry.event_type == "REGISTER_FLOOD"
    ...
    for ip, times in ip_events.items():
        ...
        for i, t in enumerate(times):
            ...
            if len(window_hits) >= THRESHOLD:
                alerts.append(Alert(
                    ip=ip,
                    attack_type="Registration Abuse",
                    severity="medium",
                    detail=f"{len(window_hits)} registration attempts in 60s",
